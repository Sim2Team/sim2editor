...
void GBASlotSelection::NextSlot() const {
	if (this->Res < 4) this->Res++;
	else this->Res = 1;
...
void GBASlotSelection::FetchSlot() const {
	...
	if (S2Core::SavUtils::Sav->SlotExist(this->Res)) {
		std::unique_ptr<S2Core::GBASlot> Slt = S2Core::SavUtils::Sav->_GBASlot(this->Res);

		this->Info.SimoleonString = S2Core::SimUtils::SimoleonsString(Slt->Simoleons());
		this->Info.RatingString = S2Core::SimUtils::RatingString(Slt->Ratings());
		this->Info.SimName = Slt->Name();
		this->Info.TimeString = S2Core::SimUtils::TimeString(Slt->Time(), false);
		this->Info.Episode = S2Core::Strings::GBAEpisodeNames_EN[Slt->CurrentEpisode()];

	} else {
		this->Info.SimoleonString = "0$";
		this->Info.RatingString = "0";
		this->Info.SimName = "";
		this->Info.TimeString = S2Core::SimUtils::TimeString(0, false);
		this->Info.Episode = "";
	}
}


void GBASlotSelection::Cancel() const {
	this->Res = -1;
	this->Done = true;
}


int8_t GBASlotSelection::Action() const {
	...
	while(!this->Done) {
		...
		Gui::DrawStringCentered(0, 3, 0.6f, TEXT_COLOR, "Select the GBASlot you like to edit.");
