...
/* Go to the previous Hair. */
void GBAHairSelection::PrevHair() const {
	if (this->Res > 0) this->Res--;
	else this->Res = 7;
}

/* Go to the next Hair. */
void GBAHairSelection::NextHair() const {
	if (this->Res < 7) this->Res++;
	else this->Res = 0;
}

/* Set the Select State. */
void GBAHairSelection::Select() const {
	this->Done = true;
}


void GBAHairSelection::Cancel() const {
	this->Res = -1;
	this->Done = true;
}


int8_t GBAHairSelection::Action() const {
	Pointer::SetPos(this->Positions[1]);
	Pointer::OnTop = true;
	if (this->Res > 7) this->Res = 7; // Just for fucks sake.
	...
	while(!this->Done) {
		...
		GFX::DrawTop();
		Gui::DrawStringCentered(0, 3, 0.6f, TEXT_COLOR, "Select the Hairstyle you want.");
		...
		Gui::DrawSprite(GFX::Sprites, sprites_frame1_idx, 195, 45); // Draw first frame of the Pointer above the current Hairstyle.

		/* Draw Hairstyle Images. */
		if (this->Res == 0) { // First Hairstyle, only draw first 2.
			Gui::DrawSprite(GFX::Appearance, this->Res, 150, 70);
			Gui::DrawSprite(GFX::Appearance, this->Res + 1, 265, 70);

		} else if (this->Res > 0 && this->Res < 7) { // 1 until 6, draw all 3.
			Gui::DrawSprite(GFX::Appearance, this->Res - 1, 35, 70);
			Gui::DrawSprite(GFX::Appearance, this->Res, 150, 70);
			Gui::DrawSprite(GFX::Appearance, this->Res + 1, 265, 70);
		...
		if (hDown & KEY_L) this->PrevHair();
		if (hDown & KEY_R) this->NextHair();
