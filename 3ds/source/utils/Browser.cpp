...
*/
Browser::Browser(const std::vector<std::string> &List) : ListEntries(List) { this->Type = BrowserType::List; }
...
*/
bool DirEntryPredicate(const DirEntry &First, const DirEntry &Second) {
...
void Browser::FetchDirectoryEntries(const std::vector<std::string> &ExtList) {
	...
	if (pdir) {
		while(true) {
			...
			if (dirEntry.Name.compare(".") != 0 && (dirEntry.IsDirectory || this->NameEndsWith(dirEntry.Name, (ExtList.empty() ? this->ExtensionList : ExtList)))) {
				this->DirEntries.push_back(dirEntry);
			}
		}
...
bool Browser::CanDirBack() {
	if (this->Type == BrowserType::File) {
		...
		if (strcmp(Path, "sdmc:/") == 0 || strcmp(Path, "/") == 0 || strcmp(Path, "sd:/") == 0 || (strcmp(Path, "fat:/") == 0)) return false;
	}
